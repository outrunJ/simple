-module(wordcount).
%% erl -pa ./ebin/ -name 'jike1@127.0.0.1'
%%  wordcount:start("./a.txt", ['jike1@127.0.0.1', 'jike2@127.0.0.1']).
...
start(FileName, NodeList) ->
  ...
  TaskResList = spawn_worker_process(file:read_line(FD), FD, NodeList, TaskOwnerNode, MainEts, []),

  [begin
     {WorkerNode, WorkerPid} = task:await(TaskRef),
     io:format(" ** One Task return it's result from Node : ~p, worker pid : ~p ~n", [WorkerNode, WorkerPid]),
     ok
...
spawn_worker_process({ok, Data}, FD, NodeList, TaskOwnerNode, MainEts, Res) ->
  ...
  TaskRef = task:async(Node, ?MODULE, worker_execute, [Data, TaskOwnerNode, MainEts]),
  spawn_worker_process(file:read_line(FD), FD, NodeList, TaskOwnerNode, MainEts, [TaskRef | Res]).
...
worker_execute(Data, TaskOwnerNode, MainEts) ->
  ...
  rpc:call(TaskOwnerNode, ?MODULE, insert_main_ets_table, [MainEts, ets:tab2list(TempEts)]),
  ets:delete(TempEts),
